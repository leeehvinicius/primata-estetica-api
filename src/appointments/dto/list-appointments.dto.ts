...
export class ListAppointmentsDto {
    ...
    @ApiProperty({ description: 'Buscar por nome do profissional', required: false })
    @IsOptional()
    @IsString()
    professionalName?: string;

    @ApiProperty({ description: 'Filtrar por tipo de agendamento', enum: AppointmentType, required: false })
    @IsOptional()
    @IsEnum(AppointmentType)
    appointmentType?: AppointmentType;

    @ApiProperty({ description: 'Filtrar por status', enum: AppointmentStatus, required: false })
    @IsOptional()
    @IsEnum(AppointmentStatus)
    status?: AppointmentStatus;

    @ApiProperty({ description: 'Filtrar por prioridade', enum: AppointmentPriority, required: false })
    @IsOptional()
    ...
    @ApiProperty({ description: 'Ordenar por campo', required: false, default: 'scheduledDate' })
    @IsOptional()
    @IsString()
