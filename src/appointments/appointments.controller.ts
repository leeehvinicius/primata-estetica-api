...
import { AppointmentResponseDto, AppointmentListResponseDto, AppointmentStatsResponseDto, AvailabilityResponseDto } from './dto/appointment-response.dto';
...
export class AppointmentsController {
    ...
    @ApiQuery({ name: 'professionalName', required: false, type: String, description: 'Buscar por nome do profissional' })
    @ApiQuery({ name: 'appointmentType', required: false, enum: ['CONSULTATION', 'TREATMENT', 'PROCEDURE', 'FOLLOW_UP', 'EMERGENCY', 'MAINTENANCE', 'EVALUATION', 'OTHER'], description: 'Filtrar por tipo de agendamento' })
    @ApiQuery({ name: 'status', required: false, enum: ['SCHEDULED', 'CONFIRMED', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED', 'NO_SHOW', 'RESCHEDULED', 'WAITING'], description: 'Filtrar por status' })
    @ApiQuery({ name: 'priority', required: false, enum: ['LOW', 'NORMAL', 'HIGH', 'URGENT'], description: 'Filtrar por prioridade' })
    ...
    @ApiResponse({ status: 200, description: 'Agendamento cancelado com sucesso', type: AppointmentResponseDto })
    @ApiResponse({ status: 404, description: 'Agendamento não encontrado' })
    ...
    @ApiResponse({ status: 200, description: 'Agendamento remarcado com sucesso', type: AppointmentResponseDto })
    @ApiResponse({ status: 404, description: 'Agendamento não encontrado' })
    @ApiResponse({ status: 400, description: 'Não é possível remarcar um agendamento cancelado' })
    ...
    @ApiResponse({ status: 200, description: 'Estatísticas dos agendamentos', type: AppointmentStatsResponseDto })
    @Get('stats/overview')
    @Roles(Role.ADMINISTRADOR, Role.MEDICO, Role.RECEPCIONISTA)
    @RequirePermission('appointments', 'read')
    @UseGuards(RolePermissionGuard)
    ...
    @ApiResponse({ status: 200, description: 'Horários disponíveis', type: AvailabilityResponseDto })
    ...
    checkAvailability(
        ...
        return this.appointments.checkAvailability(date, startTime, endTime, professionalId, serviceId);
    }
}
