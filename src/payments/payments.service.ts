...
export class PaymentsService {
    ...
    if (dto.appointmentId && appointmentData) {
      if (appointmentData.professional) {
        await this.calculateCommission(
          payment.id,
          appointmentData.professional.id,
          finalAmount,
          userId,
        );
      }
      ...
      const partnerId =
        appointmentData.partnerId || appointmentData.partner?.id;
      ...
      if (appointmentData.partner && !dto.partnerDiscount) {
        const partnerDiscountFromPartner =
          Number(appointmentData.partner.partnerDiscount) || 0;
      ...
      if (partnerId && partnerDiscount > 0) {
        console.log(
          `Creating partner commission: partnerId=${partnerId}, amount=${Number(dto.amount)}, discount=${partnerDiscount}`,
        );
        ...
        console.log(
          `Skipping partner commission: partnerId=${partnerId}, partnerDiscount=${partnerDiscount}`,
        );
      }
    }
  ...
  async update(id: string, dto: UpdatePaymentDto) {
    ...
    if (dto.partnerDiscount !== undefined)
      updateData.partnerDiscount = dto.partnerDiscount;
    if (dto.clientDiscount !== undefined)
      updateData.clientDiscount = dto.clientDiscount;
    ...
    if (
      ...
      dto.partnerDiscount !== undefined ||
      dto.clientDiscount !== undefined ||
      dto.additionalValue !== undefined
    ) {
      const newAmount = dto.amount ?? Number(existingPayment.amount);
      const newPartnerDiscount =
        dto.partnerDiscount ?? Number(existingPayment.partnerDiscount);
      const newClientDiscount =
        dto.clientDiscount ?? Number(existingPayment.clientDiscount);
      ...
      const newAdditionalValue =
        dto.additionalValue ??
        Number((existingPayment as any).additionalValue || 0);
  ...
  async getStats() {
    ...
    const averageAmount =
      total > 0 ? Number(totalAmount._sum.finalAmount || 0) / total : 0;
  ...
  private async calculatePartnerCommission(
    ...
    const partnerCommissionAmount =
      (baseAmount * partnerDiscountPercentage) / 100;

    try {
      // Verificar se o modelo existe no Prisma Client
      if (!(this.prisma as any).partnerCommission) {
        console.warn(
          'PartnerCommission model not found in Prisma Client. Make sure to run: npx prisma generate && npx prisma migrate dev',
        );
        return;
      }
      ...
      if (
        error.message?.includes('partnerCommission') ||
        error.message?.includes('PartnerCommission')
      ) {
        console.error(
          'Error creating partner commission. Make sure the migration was applied:',
          error.message,
        );
      } else {
        throw error;
  ...
  async getPartnerCommissions(
    ...
    const totalAmount = commissions.reduce(
      (sum: number, comm: any) => sum + Number(comm.amount),
      0,
    );
