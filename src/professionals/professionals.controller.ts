...
import { ProfessionalResponseDto, ProfessionalListResponseDto, ProfessionalStatsResponseDto } from './dto/professional-response.dto';
...
export class ProfessionalsController {
    ...
    @ApiResponse({ status: 201, description: 'Profissional criado com sucesso', type: ProfessionalResponseDto })
    ...
    @ApiResponse({ status: 200, description: 'Lista de profissionais', type: ProfessionalListResponseDto })
    @Get()
    ...
    @ApiQuery({ name: 'search', required: false, type: String, description: 'Buscar por nome, email, telefone, especialidade ou documento' })
    @ApiQuery({ name: 'specialty', required: false, type: String, description: 'Filtrar por especialidade' })
    ...
    @ApiResponse({ status: 200, description: 'Dados do profissional', type: ProfessionalResponseDto })
    ...
    @ApiResponse({ status: 200, description: 'Profissional atualizado com sucesso', type: ProfessionalResponseDto })
    @ApiResponse({ status: 404, description: 'Profissional não encontrado' })
    @ApiResponse({ status: 409, description: 'Email ou CPF já cadastrado' })
    @Put(':id')
    @Roles(Role.ADMINISTRADOR)
    ...
    @ApiResponse({ status: 200, description: 'Status alterado com sucesso', type: ProfessionalResponseDto })
    @ApiResponse({ status: 404, description: 'Profissional não encontrado' })
    ...
    @ApiResponse({ status: 200, description: 'Estatísticas dos profissionais', type: ProfessionalStatsResponseDto })
    @Get('stats/overview')
    @Roles(Role.ADMINISTRADOR, Role.MEDICO, Role.RECEPCIONISTA)
    @RequirePermission('professionals', 'read')
    @UseGuards(RolePermissionGuard)
    ...
    @ApiResponse({ status: 200, description: 'Lista de profissionais por especialidade' })
    ...
    @ApiResponse({ status: 200, description: 'Lista de profissionais disponíveis' })
    @Get('available/:date')
    @Roles(Role.ADMINISTRADOR, Role.MEDICO, Role.RECEPCIONISTA)
    @RequirePermission('professionals', 'read')
    @UseGuards(RolePermissionGuard)
