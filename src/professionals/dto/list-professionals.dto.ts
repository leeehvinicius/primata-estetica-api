...
export class ListProfessionalsDto {
    ...
    @ApiProperty({ description: 'Buscar por nome, email, telefone, especialidade ou documento', required: false })
    @IsOptional()
    @IsString()
    search?: string;
    ...
    @ApiProperty({ description: 'Filtrar por gênero', enum: Gender, required: false })
    @IsOptional()
    @IsEnum(Gender)
    gender?: Gender;
