...
import { AppointmentReportDto, ServiceAnalysisDto, AppointmentReportType } from './dto/appointment-report.dto';
...
export class ReportsController {
    ...
    @ApiResponse({ status: 200, description: 'Relatório financeiro gerado com sucesso' })
    @Post('financial')
    ...
    @RequirePermission('reports', 'read')
    @UseGuards(RolePermissionGuard)
    generateProfessionalPerformanceReport(@Body() dto: ProfessionalPerformanceDto) {
    ...
    @ApiResponse({ status: 200, description: 'Relatório de agendamentos gerado com sucesso' })
    @Post('appointments')
    @Roles(Role.ADMINISTRADOR, Role.MEDICO, Role.RECEPCIONISTA)
    @RequirePermission('reports', 'read')
    @UseGuards(RolePermissionGuard)
    ...
    @ApiResponse({ status: 200, description: 'Análise de serviços gerada com sucesso' })
    ...
    @ApiResponse({ status: 200, description: 'Relatório de agendamentos da semana' })
    ...
    @RequirePermission('reports', 'read')
    @UseGuards(RolePermissionGuard)
    @ApiQuery({ name: 'topN', required: false, type: Number, description: 'Número de serviços (padrão: 10)', default: 10 })
    ...
    @ApiResponse({ status: 200, description: 'Desempenho dos profissionais do mês' })
