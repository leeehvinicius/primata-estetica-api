...
export class ReportsService {
  ...
  async generateFinancialReport(dto: FinancialReportDto) {
    const {
      ...
      includeProfessionalDetails,
      includePaymentMethodDetails,
    } = dto;
    ...
    const totalRevenue = payments.reduce(
      (sum: number, payment: any) => sum + Number(payment.finalAmount),
      0,
    );
    const totalDiscounts = payments.reduce((sum: number, payment: any) => {
      const partnerDiscountAmount =
        (Number(payment.amount) * Number(payment.partnerDiscount || 0)) / 100;
      const clientDiscountAmount =
        (Number(payment.amount) * Number(payment.clientDiscount || 0)) / 100;
      ...
      serviceDetails = await Promise.all(
        serviceStats.map(async (stat: any) => {
          ...
          return {
            ...
            averageValue:
              stat._count.serviceId > 0
                ? Number(stat._sum.finalAmount) / stat._count.serviceId
                : 0,
          };
        }),
      );
      ...
      professionalDetails = await Promise.all(
        professionalStats.map(async (stat: any) => {
          ...
          return {
            ...
            averageCommission:
              stat._count.professionalId > 0
                ? Number(stat._sum.amount) / stat._count.professionalId
                : 0,
          };
        }),
      );
  ...
  async generateProfessionalPerformanceReport(dto: ProfessionalPerformanceDto) {
    ...
    const averageAppointmentsPerDay =
      daysDiff > 0 ? totalAppointments / daysDiff : 0;
  ...
  async generateServiceAnalysis(dto: ServiceAnalysisDto) {
    ...
    const highestRevenueServices = await Promise.all(
      revenueStats
        .sort(
          (a: any, b: any) =>
            Number(b._sum.finalAmount) - Number(a._sum.finalAmount),
        )
        .slice(0, topN)
        .map(async (stat: any) => {
          const service = await (this.prisma as any).service.findUnique({
  ...
  async getDashboard() {
    ...
    const averageTicket =
      totalPayments > 0 ? totalRevenueValue / totalPayments : 0;
    ...
    const topProfessionalsWithNames = await Promise.all(
      topProfessionals.map(async (professional: any) => {
        ...
        return {
          ...
          professionalName:
            professionalData?.name || 'Profissional não encontrado',
