...
export class EnhancedPermissionsService {
      ...
      case SecurityResource.APPOINTMENTS:
        if (dto.resourceId) {
          ...
          if (appointment && appointment.professionalId === user.id) {
            return {
              allowed: true,
              reason: 'Agendamento atribuído ao profissional',
            };
          }
        }
        return {
  ...
  private async evaluateCondition(
    ...
    switch (condition) {
      case 'patient_assigned':
        ...
        if (dto.resourceId) {
          const assignment = await this.checkPatientAssignment(
            dto.resourceId,
            user.id,
          );
          return assignment
      ...
      case 'active_treatment':
        ...
        if (dto.resourceId) {
          const activeTreatment = await this.checkActiveTreatment(
            dto.resourceId,
            user.id,
          );
      ...
      case 'professional_assigned':
        ...
        if (dto.resourceId) {
          const assignment = await this.checkProfessionalAssignment(
            dto.resourceId,
            user.id,
          );
          return assignment
      ...
      case 'cleaning_products_only':
        ...
        if (dto.resourceId) {
          const isCleaningProduct = await this.checkCleaningProduct(
            dto.resourceId,
          );
  ...
  private async checkRestrictions(
    ...
    for (const restriction of permission.restrictions) {
      const restrictionResult = await this.evaluateRestriction(
        restriction,
        dto,
        user,
      );
  ...
  private async evaluateRestriction(
    ...
    switch (restriction) {
      ...
      case 'basic_reports_only':
        ...
        if (
          dto.context &&
          dto.context.reportType &&
          !this.isBasicReport(dto.context.reportType)
        ) {
          return {
            allowed: false,
  ...
  private checkTimeRestrictions(
    ...
    const match = permission.timeRestriction.match(
      /(\d{2}):(\d{2})-(\d{2}):(\d{2})/,
    );
    if (!match) {
      return {
        allowed: true,
        reason: 'Formato de restrição de horário inválido',
      };
    }
  ...
  private async checkActiveTreatment(
    clientId: string,
    professionalId: string,
  ): Promise<boolean> {
  ...
  private async checkProfessionalAssignment(
  ...
  async getUserPermissions(userId: string): Promise<EnhancedPermission[]> {
    ...
    return ENHANCED_ROLE_PERMISSIONS[user.profile.role] || [];
  }
