...
export class BackupService {
  ...
  async createBackup(
    ...
    try {
      ...
      if (defaultOptions.encrypt) {
        finalData = this.encryptionService.encryptSensitiveData(
          finalData,
          'backup',
        );
        isEncrypted = true;
  ...
  async restoreBackup(backupId: string, userId: string): Promise<void> {
    try {
      ...
      if (backupInfo.encrypted) {
        fileContent = this.encryptionService.decryptSensitiveData(
          fileContent,
          'backup',
        );
      }
      ...
      this.logger.warn(
        'RESTAURAÇÃO DE BACKUP - Funcionalidade requer implementação cuidadosa',
      );
    ...
    try {
      ...
      for (const file of backupFiles) {
        try {
          ...
          this.logger.warn(
            `Erro ao processar arquivo de backup: ${file}`,
            error,
          );
        }
      }
  ...
  private async generateBackupData(options: BackupOptions): Promise<any> {
    ...
    if (options.includeSecurityEvents) {
      try {
        ...
        this.logger.warn(
          'Erro ao fazer backup dos eventos de segurança',
          error,
        );
      }
    }
  ...
  private async getBackupInfo(backupId: string): Promise<BackupInfo | null> {
    try {
      const config = await this.securityService.getSecurityConfig(
        `backup_${backupId}`,
      );
      ...
      for (const backup of backups) {
        if (backup.createdAt < cutoffDate) {
          try {
            ...
            this.logger.warn(
              `Erro ao remover backup antigo: ${backup.filename}`,
              error,
            );
          }
        }
