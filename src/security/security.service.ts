...
export class SecurityService {
  ...
  async logSecurityEvent(params: {
    ...
    try {
      ...
      if (
        params.severity === SecuritySeverity.CRITICAL ||
        params.severity === SecuritySeverity.HIGH
      ) {
        this.logger.error(
          `SECURITY EVENT: ${params.eventType} - ${params.description}`,
          {
            userId: params.userId,
            ipAddress: params.ipAddress,
  ...
  async createSession(params: {
    ...
    const refreshTokenHash = params.refreshToken
      ? this.hashToken(params.refreshToken)
      : null;
    const location = this.getLocationFromIP(params.ipAddress);
