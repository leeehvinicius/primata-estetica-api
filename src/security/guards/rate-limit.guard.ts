...
export class RateLimitGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        if (entry.count > rateLimitConfig.maxRequests) {
            const { ipAddress, userAgent } = this.securityService.extractRequestInfo(request);
            ...
            throw new HttpException(
                rateLimitConfig.message || `Muitas requisições. Tente novamente em ${timeUntilReset} segundos.`,
