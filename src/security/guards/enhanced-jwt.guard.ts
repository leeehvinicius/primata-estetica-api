...
export class EnhancedJwtGuard extends AuthGuard('jwt') {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!session) {
      await this.securityService.logSecurityEvent({
        ...
        severity: 'HIGH',
        description:
          'Tentativa de acesso com token inválido ou sessão expirada',
  ...
  private async checkSuspiciousActivity(request: Request, session: any) {
    const currentIP =
      this.securityService.extractRequestInfo(request).ipAddress;
    ...
    if (currentIP !== sessionIP) {
      ...
      if (!session.trusted) {
        ...
        throw new UnauthorizedException(
          'Sessão invalidada por atividade suspeita',
        );
      }
    }
