...
export class StockService {
  ...
  async updateCategory(id: string, dto: UpdateProductCategoryDto) {
    const existing = await (this.prisma as any).productCategory.findUnique({
      where: { id },
    });
  ...
  async createProduct(dto: CreateProductDto, userId: string) {
    ...
    if (dto.expirationDate)
      productData.expirationDate = new Date(dto.expirationDate);
  ...
  async updateProduct(id: string, dto: UpdateProductDto) {
    ...
    if (dto.sku && dto.sku !== existingProduct.sku) {
      const existingProductWithSku = await (
        this.prisma as any
      ).product.findUnique({
        where: { sku: dto.sku },
      });
    ...
    if (dto.barcode && dto.barcode !== existingProduct.barcode) {
      const existingProductWithBarcode = await (
        this.prisma as any
      ).product.findUnique({
        where: { barcode: dto.barcode },
      });
  ...
  async getStockStats() {
    ...
    const totalValue =
      totalStockValue._sum.currentStock * (totalStockValue._sum.costPrice || 0);
