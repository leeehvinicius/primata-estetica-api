...
export class CreateServiceDto {
    ...
    @ApiProperty({ description: 'Se o serviço requer profissional', required: false, default: true })
    @IsOptional()
    @IsBoolean()
    requiresProfessional?: boolean;

    @ApiProperty({ description: 'Máximo de clientes simultâneos', required: false, default: 1 })
    @IsOptional()
    @IsNumber()
    @Min(1)
    ...
    @ApiProperty({ description: 'Status ativo do serviço', required: false, default: true })
    @IsOptional()
    @IsBoolean()
    isActive?: boolean;
}
