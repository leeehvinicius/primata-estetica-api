...
export class WebhookService {
  ...
  async sendPaymentWebhook(paymentId: string, paymentData: any) {
    try {
      ...
      const webhookData = this.preparePaymentWebhookData(
        paymentId,
        paymentData,
      );
  ...
  async sendAppointmentWebhook(
    ...
    try {
      this.logger.log(
        `Enviando webhook de agendamento ${appointmentId} - ${event}`,
      );
      ...
      const webhookData = this.prepareAppointmentWebhookData(
        appointmentId,
        appointmentData,
        event,
      );
  ...
  async sendClientWebhook(clientId: string, clientData: any, event: string) {
    try {
      ...
      const webhookData = this.prepareClientWebhookData(
        clientId,
        clientData,
        event,
      );
  ...
  private prepareAppointmentWebhookData(
    appointmentId: string,
    appointmentData: any,
    event: string,
  ) {
  ...
  private prepareClientWebhookData(
    clientId: string,
    clientData: any,
    event: string,
  ) {
  ...
  private verifyWebhookSignature(data: any, signature: string): boolean {
    ...
    if (!secret) {
      this.logger.warn(
        'WEBHOOK_SECRET não configurado, pulando verificação de assinatura',
      );
      return true;
    }
