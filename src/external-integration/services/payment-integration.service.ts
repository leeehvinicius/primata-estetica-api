...
export class PaymentIntegrationService {
  ...
  async processPayment(paymentData: {
    ...
    try {
      this.logger.log(
        `Processando pagamento ${paymentData.internalPaymentId} via gateway externo`,
      );
      ...
      if (!gatewayConfig) {
        throw new Error(
          `Gateway não configurado para método ${paymentData.paymentMethod}`,
        );
      }
      ...
      // Processar resposta
      const result = this.processGatewayResponse(
        response,
  ...
  async refundPayment(paymentId: string, amount?: number, reason?: string) {
    try {
      ...
      const response = await this.sendRefundToGateway(
        gatewayConfig,
        refundData,
      );

      // Processar resposta
      const result = this.processRefundResponse(
        response,
        gatewayConfig.provider,
      );
  ...
  async checkPaymentStatus(paymentId: string) {
    try {
      ...
      const response = await this.checkStatusWithGateway(
        gatewayConfig,
  ...
  private async sendToGateway(gatewayConfig: PaymentGatewayConfig, data: any) {
    ...
    try {
      ...
      this.logger.error(
        `Erro na comunicação com gateway ${gatewayConfig.provider}`,
        error,
      );
  ...
  private async sendRefundToGateway(
    ...
    try {
      ...
      this.logger.error(
        `Erro no reembolso via gateway ${gatewayConfig.provider}`,
        error,
      );
  ...
  private async checkStatusWithGateway(
    gatewayConfig: PaymentGatewayConfig,
    ...
    try {
      ...
      this.logger.error(
        `Erro ao verificar status via gateway ${gatewayConfig.provider}`,
        error,
      );
  ...
  private buildGatewayUrl(
    ...
    switch (config.provider) {
      case 'stripe':
        const baseUrl = settings.sandbox
        ...
        switch (action) {
          ...
          case 'status':
            return `${baseUrl}/payment_intents/${transactionId}`;
          case 'test':
      ...
      case 'mercadopago':
        const mpBaseUrl = settings.sandbox
          ? 'https://api.mercadopago.com'
          : 'https://api.mercadopago.com';
        switch (action) {
          case 'payment':
            return `${mpBaseUrl}/v1/payments`;
          case 'refund':
            return `${mpBaseUrl}/v1/payments/${transactionId}/refunds`;
          case 'status':
            return `${mpBaseUrl}/v1/payments/${transactionId}`;
          case 'test':
            return `${mpBaseUrl}/users/me`;
          default:
            return mpBaseUrl;
        }
      case 'paypal':
        const ppBaseUrl = settings.sandbox
          ? 'https://api-m.sandbox.paypal.com'
          : 'https://api-m.paypal.com';
        switch (action) {
          case 'payment':
            return `${ppBaseUrl}/v2/checkout/orders`;
          case 'refund':
            return `${ppBaseUrl}/v2/payments/captures/${transactionId}/refund`;
          case 'status':
            return `${ppBaseUrl}/v2/checkout/orders/${transactionId}`;
          case 'test':
            return `${ppBaseUrl}/v1/identity/oauth2/userinfo`;
          default:
            return ppBaseUrl;
        }
      default:
  ...
  private processStatusResponse(response: any, provider: string) {
    switch (provider) {
      ...
      case 'paypal':
        return {
          status:
            response.status === 'COMPLETED' ? 'COMPLETED' : response.status,
