...
export class ApiIntegrationService {
  ...
  async makeApiRequest(config: {
    ...
    try {
      this.logger.log(
        `Fazendo requisição para API externa: ${config.method} ${config.endpoint}`,
      );

      // Obter configuração da integração
      const integrationConfig =
        await this.prisma.integrationConfiguration.findUnique({
  ...
  async syncDataWithApi(syncConfig: {
    ...
    try {
      this.logger.log(
        `Sincronizando ${syncConfig.data.length} registros com API externa`,
      );
      ...
      for (let i = 0; i < batches.length; i++) {
        ...
        this.logger.log(
          `Processando lote ${i + 1}/${batches.length} (${batch.length} registros)`,
        );
  ...
  async importDataFromApi(importConfig: {
    ...
    try {
      this.logger.log(
        `Importando dados de API externa: ${importConfig.dataType}`,
      );
      ...
      if (importConfig.transformFunction) {
        transformedData = this.transformData(
          response.data,
          importConfig.transformFunction,
        );
      }
  ...
  async checkApiStatus(integrationId: string) {
    try {
      ...
      const testResponse = await this.makeApiRequest({
        ...
        method: 'GET',
        endpoint:
          (integrationConfig.settings as ApiSettings)?.healthEndpoint ||
          '/health',
      });
      ...
      this.logger.error(
        `Erro ao verificar status da API ${integrationId}`,
        error,
      );
      return {
        success: false,
  ...
  async testApiConnection(integrationId: string) {
    try {
      ...
      const testResponse = await this.makeApiRequest({
        ...
        method: 'GET',
        endpoint:
          (integrationConfig.settings as ApiSettings)?.testEndpoint || '/',
      });
      ...
      this.logger.error(
        `Erro no teste de conexão com API ${integrationId}`,
        error,
      );
      return {
        success: false,
  ...
  async getApiUsageStats(
    ...
    try {
      ...
      const stats = {
        ...
        successfulRequests: logs.filter((log) => log.status === 'SUCCESS')
          .length,
  ...
  private buildApiHeaders(integrationConfig: any, customHeaders?: any) {
    ...
    if (integrationConfig.credentials?.bearerToken) {
      headers['Authorization'] =
        `Bearer ${integrationConfig.credentials.bearerToken}`;
    }
