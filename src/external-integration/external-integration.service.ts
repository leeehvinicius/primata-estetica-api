...
export class ExternalIntegrationService {
    ...
    async processPayment(paymentData: {
        ...
        try {
            this.logger.log(`Processando pagamento para cliente ${paymentData.clientId}`);
    ...
    async exportFinancialReports(period: {
        ...
        try {
            this.logger.log(`Exportando relatórios financeiros para ${period.startDate} até ${period.endDate}`);
            ...
            const report = await this.accountingIntegration.generateFinancialReport(payments, period.format);
    ...
    async configureIntegration(config: {
        ...
        try {
            this.logger.log(`Configurando integração ${config.type} com ${config.provider}`);
